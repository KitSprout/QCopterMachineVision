...
int main( void )
{
  ...
  while(KEY != 1){
	...
	// Read Directory File
  RS232_Print(USART3, (u8*)"----------------------\r\n");
  RS232_Print(USART3, (u8*)" SD_Card Read Directory File\r\n");
  RS232_Print(USART3, (u8*)"----------------------\r\n\r\n");
  ...
  while(res!= FR_OK) {
		RS232_Print(USART3, (u8*)" Fatfs failed!!\r\n");
		while(1) {
      LED_R = ~LED_R;
      Delay_100ms(2);
    }
	}
  RS232_Print(USART3, (u8*)" File name : \r\n");
  ...
  while(finfo.fname[0]) {
    ...
    if(!finfo.fname[0])
      break;
    RS232_Print(USART3, (u8*)" ... ");
    RS232_Print(USART3, (u8*)finfo.fname);
    RS232_Print(USART3, (u8*)"\r\n");
  }
